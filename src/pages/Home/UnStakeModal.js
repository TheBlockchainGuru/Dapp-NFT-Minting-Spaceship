...
const UnStakeModal = ({open, onClose, nfts, setNft}) => {
    let index = 0
    ...
    const onToggle = (e, nftIndex) => {
        if(originNfts[nftIndex].state == 1) {
    ...
    return (
        ...
        >
            ...
            <div style={{
                ...
                {
                    originNfts.length && originNfts.find(ele => ele.state == 1)
                    ?
                    originNfts.map(e => 
                            <div key={index ++}>
                                {e.state == 1 
